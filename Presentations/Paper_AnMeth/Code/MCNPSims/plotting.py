...
bbbbbox = dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.8)
...
def plot_box(ax, xmin, xmax, ymin, ymax, zmin, zmax, color, alpha=1, label=None, zorder=None):
    ...
    if label:
        ax.text((xmin+xmax)/2, (ymin+ymax)/2, zmax, label, color=color, zorder=zorder, bbox=bbbbbox)
...
def plot_cylinder(ax, base, vec, radius, height, color, alpha=1, label=None, zorder=None):
    ...
    if label:
        ax.text(x0, y0, z0+height/2, label, color=color, bbox=bbbbbox)
...
def plot_cone(ax, pos, vec, dir, length, color='red', alpha=1, label=None, zorder=None):
    ...
    if label:
        ax.text(x0, y0, z0 + dz * length / 2, label, color=color,  bbox=bbbbbox)
...
def plot_MCNP(ax, zorder=100000):
    # Detector (surface 21)
    plot_cylinder(ax, base=(56, -5.0, -1.0), vec=(0.0, 20.3, 0.0), radius=4.5, height=20.3, color='red', alpha=1, label='Detector', zorder=zorder)

    # Shielding boxes (RPPs)
    plot_box(ax, 19, 29, -7.5, 7.5, -11, 9, color='blue', alpha=1, label='PbPE', zorder=zorder)
    plot_box(ax, 9, 19, 4, 9, -11, 9, color='cyan', alpha=1, zorder=zorder)
    plot_box(ax, 9, 19, -9, -4, -11, 9, color='cyan', alpha=1, zorder=zorder)
    plot_box(ax, 19, 29, 7.5, 12.5, -11, 9, color='cyan', alpha=1, zorder=zorder)
    plot_box(ax, 19, 29, -12.5, -7.5, -11, 9, color='cyan', alpha=1, zorder=zorder)
    plot_box(ax, 29, 34, -15, 15, -11, 9, color='cyan', alpha=1, zorder=zorder)
    plot_box(ax, 9, 19, -4, 4, 4, 9, color='cyan', alpha=1, zorder=zorder)
    plot_box(ax, -26, 26, 18, 28, -11, 9, color='green', alpha=1, label='BA1', zorder=zorder)
    plot_box(ax, -26, 26, -28, -18, -11, 9, color='green', alpha=1, label='BA2', zorder=zorder)
    plot_box(ax, -65, 65, -28, 28, 10, 10.5, color='magenta', alpha=1, label='Al', zorder=zorder)

    # Wheels (outer treads only for clarity)
    plot_cylinder(ax, base=(-2, 77, 8), vec=(0, 25, 0), radius=29, height=25, color='orange', alpha=0.2, label='Wheel 1', zorder=zorder)
    plot_cylinder(ax, base=(68, 77, 8), vec=(0, 25, 0), radius=29, height=25, color='orange', alpha=0.2, label='Wheel 2', zorder=zorder)
    plot_cylinder(ax, base=(-2, -77, 8), vec=(0, -25, 0), radius=29, height=25, color='orange', alpha=0.2, label='Wheel 3', zorder=zorder)
    plot_cylinder(ax, base=(68, -77, 8), vec=(0, -25, 0), radius=29, height=25, color='orange', alpha=0.2, label='Wheel 4', zorder=zorder)

    # Soil volume (bounding box)
    # plot_box(ax, -56, 56, -45, 45, 42, 92, color='brown', alpha=1, label='Soil', zorder=zorder)
